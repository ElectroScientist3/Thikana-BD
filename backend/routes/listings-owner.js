...
router.get('/dashboard/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const available = await Listing.countDocuments({ 
      owner_id: req.userId,
      status: 'available_now'
    });
    const rented = await Listing.countDocuments({ 
      owner_id: req.userId,
      status: 'rented'
    });
...
router.patch('/bulk-status', authMiddleware, async (req, res) => {
  try {
    ...
    // Verify all listings belong to this owner
...
router.get('/available-tenants', authMiddleware, async (req, res) => {
  try {
    // This would typically be a separate tenant management system
    // For now, we'll return a simple list of users who might be looking
