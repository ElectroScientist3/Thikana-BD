...
function Payment() {
  ...
  useEffect(() => {
    if (!autoBookingStarted && bookingIdParam) {
      const booking = bookings.find((item) => item.id === bookingIdParam);
      if (booking) {
        setAutoBookingStarted(true);
        handleBookingPayment(booking);
      } else {
        setStatusMessage(`Could not find booking ${bookingIdParam}`);
        setStatusTone("bg-amber-100 text-amber-700");
      }
    }
  ...
  const handleBookingPayment = async (booking) => {
    ...
    try {
      const raw = booking.amount.replace(/[^0-9.]/g, "");
      ...
      const response = await fetch(`${apiBase}/api/payments/initiate`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ amount, purpose: "booking", bookingId: booking.id, name: booking.tenant }),
      });
      const data = await parseApiResponse(response);
      if (response.status === 401 || response.status === 403) {
